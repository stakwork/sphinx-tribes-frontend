...
interface TicketEditorProps {
  ...
  onTicketSaved?: (ticketUuid: string) => void;
}
...
const NewTicketEditor = observer(
  ({
    ticketData,
    websocketSessionId,
    dragHandleProps,
    ...
    const latestTicket = phaseTicketStore.getLatestVersionFromGroup(
      ticketData?.ticket_group || ticketData?.uuid
    );
    const [selectedVersion, setSelectedVersion] = useState<number>(latestTicket?.version || 1);
    const [versionTicketData, setVersionTicketData] = useState<Ticket>(latestTicket || ticketData);
    ...
    const groupTickets = useMemo(
      () => phaseTicketStore.getTicketsByGroup(ticketData?.ticket_group || ticketData?.uuid),
      [ticketData?.ticket_group, ticketData?.uuid]
    );
    ...
    useEffect(() => {
      const maxLimit = 21;
      const latestVersion = latestTicket?.version || 1;
    ...
    const handleUpdate = async () => {
      ...
      try {
        ...
        if (response.UUID && onTicketSaved) {
          console.log('Ticket created with UUID:', response.UUID);
          onTicketSaved(response.UUID);
        } else {
          console.error('No UUID in response:', response);
        }
    ...
    const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (version !== selectedVersion) {
        ...
        const selectedVersionTicket = groupTickets.find(
          (ticket: Ticket) => ticket.version === version
        );

        if (selectedVersionTicket) {
          setVersionTicketData({
            ...selectedVersionTicket
          });
        }
      }
    };
...
export default NewTicketEditor;
