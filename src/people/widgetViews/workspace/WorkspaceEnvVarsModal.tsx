...
const WorkspaceEnvVarsModal: React.FC<WorkspaceEnvVarsModalProps> = observer(({ open, onClose, workspaceUuid }) => {
  ...
  useEffect(() => {
    if (open && workspaceUuid) {
      setLoading(true);
      mainStore.getWorkspaceEnvVars(workspaceUuid)
  ...
  return (
    <div className="modal-backdrop" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.3)', zIndex: 1000 }}>
      <div className="modal" style={{ background: '#fff', margin: '5% auto', padding: 24, borderRadius: 8, maxWidth: 600, minWidth: 320 }}>
            ...
            <tbody>
              {envVars.map((v, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <button onClick={() => handleToggleShow(idx)} title={v._show ? 'Hide' : 'Show'}>
                      {v._show ? '🙈' : '👁️'}
                    </button>
                  </td>
                  <td>
        ...
        <button onClick={handleAdd} style={{ marginRight: 8 }}>Add</button>
