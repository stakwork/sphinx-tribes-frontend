...
describe("User Tickets View", () => {
	...
	it("should redirect to bounty page when bounty card is clicked", async () => {
		...
		await act(async () => {
			...
			expect(
				getAllByTestId("user-personal-bounty-card")[0].getAttribute("href"),
			).toEqual(`/p/1234/usertickets/${userBounty.body.id}/0`);
		});
	});
	...
	it("should render status assigned if ticket is assigned", async () => {
		...
		await act(async () => {
			...
			expect(getAllByTestId("user-personal-bounty-card").length).toBe(1);
			within(
				within(getAllByTestId("user-personal-bounty-card")[0]).getByTestId(
					"status-pill",
				),
			).getByText("Assigned");
		});
