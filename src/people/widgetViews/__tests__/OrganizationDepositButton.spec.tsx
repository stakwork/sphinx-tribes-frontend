...
describe('Organization Details component', () => {
    ...
    it('Renders Deposit Modal', async () => {
        const {getByText, container} = render(<OrganizationDetails close={closeFn} getOrganizations={getOrgFn} org={organization} resetOrg={resetOrgFn} />) 
    ...
    it('Test that USD conversion are visible', async () => {
      const {getByTestId, container} = render(<OrganizationDetails close={closeFn} getOrganizations={getOrgFn} org={organization} resetOrg={resetOrgFn} />)
      fireEvent.click(screen.getByText("Deposit"));
    ...
    it('User can input an amount for sats', async () => {
      const {getByText, getByTestId, container} = render(<OrganizationDetails close={closeFn} getOrganizations={getOrgFn} org={organization} resetOrg={resetOrgFn} />)
      fireEvent.click(screen.getByText("Deposit"));
         waitFor(() => {
    ...
    it('Invoice is displayed in a QR code', async () => {
      const {getByTestId} = render(<OrganizationDetails close={closeFn} getOrganizations={getOrgFn} org={organization} resetOrg={resetOrgFn} />)
      fireEvent.click(screen.getByText("Deposit"));
      waitFor(() => {
        const satInput = getByTestId("input-amount")
        fireEvent.change(satInput, { target: { value: '10' } });
