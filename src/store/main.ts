...
export class MainStore {
  ...
  async getUserFeatureByUuid(uuid: string): Promise<Feature | undefined> {
    try {
      if (!uiStore.meInfo) return undefined;
      const info = uiStore.meInfo;
      console.log('TribesURL', TribesURL);
      ...
      console.log('Error getUserFeatureByUuid', e);
      return undefined;
    }
  }

  @action async addOrUpdateFeature(body: CreateFeatureInput): Promise<any> {
    try {
      if (!uiStore.meInfo) return null;
      const info = uiStore.meInfo;
      ...
      console.log('Error addOrUpdateFeature', e);
      return false;
    }
  }
